...
- external corroboration from a live meta reference when a patch already shows fast adoption on affected picks
...
The scheduled workflow also syncs an external live-meta snapshot.

It is used as a **secondary** source:

- Riot patch notes remain the primary source for official change detection
- the external live-meta layer helps confirm rapid live adoption, tier shifts, and suspicious item shells
...
- “the external feed says it, therefore it is final truth”
