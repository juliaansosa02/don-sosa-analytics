...
const focusWeightMapSchema = z.object({
  ...
  lane_volatility: z.number().min(-1).max(1).optional(),
  reset_timing: z.number().min(-1).max(1).optional(),
  objective_setup_quality: z.number().min(-1).max(1).optional(),
  champion_identity_execution: z.number().min(-1).max(1).optional(),
  itemization_read: z.number().min(-1).max(1).optional(),
  rune_setup_read: z.number().min(-1).max(1).optional(),
