...
const rankCrestTierConfig: Record<string, { directory: string; slug: string }> = {
...
function extractRankDivision(label?: string | null) {
  if (!label) return null;
  const match = label.toUpperCase().match(/\b(IV|III|II|I)\b/);
  if (!match) return null;
  const divisionMap: Record<string, number> = { I: 1, II: 2, III: 3, IV: 4 };
  return divisionMap[match[1]] ?? null;
}

export function getRankCrestLayerUrls(tier?: string, label?: string | null) {
