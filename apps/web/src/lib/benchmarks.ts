...
export interface TierComparisonPoint {
  tier: string;
  value: number;
}

export interface TierProgressionBenchmark {
  currentTier: TierComparisonPoint;
  nextTier: TierComparisonPoint | null;
...
const level15Benchmarks: Record<string, Record<string, number>> = {
...
function getNextTier(tier?: string | null) {
  const normalizedTier = normalizeTier(tier ?? undefined);
  if (!normalizedTier) return null;
  const currentIndex = tierOrder.indexOf(normalizedTier as (typeof tierOrder)[number]);
  if (currentIndex === -1 || currentIndex === tierOrder.length - 1) return null;
  return tierOrder[currentIndex + 1];
}

function buildTierProgression(role: string, tier: string | null | undefined, actualValue: number | undefined, benchmarks: Record<string, Record<string, number>>) {
  const normalizedTier = normalizeTier(tier ?? undefined);
  if (!normalizedTier || typeof actualValue !== 'number') return null;
  const byRole = benchmarks[role];
  if (!byRole) return null;
  const currentTierValue = byRole[normalizedTier];
  if (typeof currentTierValue !== 'number') return null;
...
export function buildCs15ProgressionBenchmark(role: string, tier?: string | null, avgCsAt15?: number) {
  return buildTierProgression(role, tier, avgCsAt15, cs15Benchmarks);
}

export function buildLevel15ProgressionBenchmark(role: string, tier?: string | null, avgLevelAt15?: number) {
