...
export const supportedRiotPlatforms = Object.keys(riotPlatformConfig);
...
export function isSupportedRiotPlatform(value) {
    const normalized = normalizeRiotPlatform(value);
    return normalized in riotPlatformConfig;
}
export function getRiotPlatformInfo(platform) {
    const normalized = normalizeRiotPlatform(platform);
    return riotPlatformConfig[normalized] ?? null;
}
export function buildProfileIdentityKey(gameName, tagLine, platform) {
    return `${normalizeRiotPlatform(platform) || 'unknown'}:${gameName.trim().toLowerCase()}#${tagLine.trim().toLowerCase()}`;
}
export function guessDefaultRiotPlatform(locale) {
    const language = typeof navigator !== 'undefined' ? navigator.language.toLowerCase() : locale;
    const timezone = typeof Intl !== 'undefined' ? Intl.DateTimeFormat().resolvedOptions().timeZone ?? '' : '';
    if (language.startsWith('ko') || timezone.includes('Seoul'))
        return 'KR';
    if (language.startsWith('ja') || timezone.includes('Tokyo'))
        return 'JP1';
    if (language.startsWith('tr') || timezone.includes('Istanbul'))
        return 'TR1';
    if (language.startsWith('ru') || timezone.includes('Moscow'))
        return 'RU';
    if (language.startsWith('pt') || timezone.includes('Sao_Paulo'))
        return 'BR1';
    if (timezone.includes('Mexico_City'))
        return 'LA1';
    if (timezone.includes('Buenos_Aires') || timezone.includes('Santiago') || timezone.includes('Lima') || timezone.includes('Bogota'))
        return 'LA2';
    if (timezone.includes('Sydney') || timezone.includes('Auckland'))
        return 'OC1';
    if (timezone.includes('Manila'))
        return 'PH2';
    if (timezone.includes('Bangkok'))
        return 'TH2';
    if (timezone.includes('Taipei') || timezone.includes('Hong_Kong'))
        return 'TW2';
    if (timezone.includes('Ho_Chi_Minh'))
        return 'VN2';
    if (timezone.includes('Singapore') || timezone.includes('Kuala_Lumpur') || timezone.includes('Jakarta'))
        return 'SG2';
    if (timezone.includes('New_York') || timezone.includes('Chicago') || timezone.includes('Los_Angeles'))
        return 'NA1';
    if (timezone.includes('Berlin') || timezone.includes('Madrid') || timezone.includes('Paris') || timezone.includes('London') || timezone.includes('Rome'))
        return 'EUW1';
    if (timezone.includes('Warsaw') || timezone.includes('Helsinki') || timezone.includes('Athens'))
        return 'EUN1';
