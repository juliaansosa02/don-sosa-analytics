...
export function getChampionAccent(championName) {
    if (!championName)
        return accentPalettes[0];
    return accentPalettes[hashText(championName) % accentPalettes.length];
}
export function formatMatchDuration(gameDurationSeconds, locale) {
...
export function findAnchorChampion(matches) {
...
export function getImpactLabel(match, locale) {
...
export function buildMatchQuickRead(match, dataset, locale) {
    ...
    const sameMatchupMatches = dataset.matches.filter((entry) => entry.championName === match.championName &&
...
export function findReferenceMatch(matches) {
    return [...matches]
        .sort((left, right) => right.gameCreation - left.gameCreation)
        .slice(0, Math.min(7, matches.length))
        .filter((match) => match.win)
        .sort((left, right) => (right.score.total + (right.timeline.goldDiffAt15 ?? 0) * 0.015 - (right.timeline.objectiveSetupScore ?? 0) * 8)
        - (left.score.total + (left.timeline.goldDiffAt15 ?? 0) * 0.015 - (left.timeline.objectiveSetupScore ?? 0) * 8))[0] ?? null;
}
export function findReviewPriorityMatch(matches) {
    return [...matches]
        .sort((left, right) => right.gameCreation - left.gameCreation)
        .slice(0, Math.min(7, matches.length))
        .filter((match) => !match.win)
        .sort((left, right) => ((right.timeline.laneVolatilityScore ?? 0) * 18 + (right.timeline.objectiveSetupScore ?? 0) * 18 + Math.max(0, 64 - right.score.total))
        - ((left.timeline.laneVolatilityScore ?? 0) * 18 + (left.timeline.objectiveSetupScore ?? 0) * 18 + Math.max(0, 64 - left.score.total)))[0] ?? null;
}
