...
function buildVariantDescriptor(match) {
  ...
  const compactLabel = secondaryMinor.length ? secondaryMinor.join(" + ") : primaryMinor.length ? primaryMinor.join(" + ") : keystone;
  return {
    key,
    keystone,
...
function describeDifferences(baseline, variant) {
  ...
  if (!changes.length) {
    return baseline.compactLabel === variant.compactLabel ? "Misma p\xE1gina, mismo \xE1rbol" : `${baseline.compactLabel} -> ${variant.compactLabel}`;
  }
...
function buildContextNote(baseline, variant, locale) {
  ...
  if (variantTop && (!baselineTop || variantTop.championName !== baselineTop.championName) && variantTop.share >= 35) {
    return copy(locale, `La variante est\xE1 m\xE1s sesgada a ${variantTop.championName} (${variantTop.share}% de la muestra).`, `The variant is more skewed toward ${variantTop.championName} (${variantTop.share}% of the sample).`);
  }
  if (Math.abs(durationGap) >= 3.5) {
    return durationGap > 0 ? copy(locale, "La variante aparece en partidas bastante m\xE1s largas que el baseline.", "The variant shows up in meaningfully longer games than the baseline.") : copy(locale, "La variante aparece en partidas bastante m\xE1s cortas que el baseline.", "The variant shows up in meaningfully shorter games than the baseline.");
  }
  if (variant.earlyStateProfile.volatile - baseline.earlyStateProfile.volatile >= 18) {
    return copy(locale, "La p\xE1gina alternativa se usa m\xE1s en earlys rotos o vol\xE1tiles.", "The alternative page appears more often in broken or volatile early games.");
  }
  if (variant.earlyStateProfile.stable - baseline.earlyStateProfile.stable >= 18) {
    return copy(locale, "La p\xE1gina alternativa se est\xE1 viendo m\xE1s en earlys limpios y controlados.", "The alternative page is showing up more often in clean and controlled early games.");
  }
  return null;
}
function buildSignalNote(baseline, variant, locale, deltas) {
  if (deltas.runeValue >= 1500 && deltas.score >= 2) {
    return copy(locale, "No cambia solo el WR: tambi\xE9n sube el valor real que la p\xE1gina extrae del kit.", "This is not only a WR shift: the page is also extracting more real value from the kit.");
  }
  ...
  if (deltas.goldDiffAt15 >= 180 && deltas.csAt15 >= 3) {
    return copy(locale, "La se\xF1al va con mejor econom\xEDa temprana y no solo con outcomes tard\xEDos.", "The signal tracks with better early economy, not only with late outcomes.");
  }
  if (deltas.runeHealing >= 250 || deltas.runeDamage >= 1200) {
    return copy(locale, "El swap est\xE1 moviendo output medible de runas y no solo sensaci\xF3n subjetiva.", "The swap is moving measurable rune output, not only subjective feel.");
  }
  return copy(locale, "La lectura principal es si el swap mejora la versi\xF3n del campe\xF3n que quer\xE9s ejecutar.", "The main read is whether the swap improves the version of the champion you want to execute.");
}
function buildConstraint(baseline, variant, locale) {
  const minSample = Math.min(baseline.games, variant.games);
  if (minSample < 4) {
    return copy(locale, "Todav\xEDa no hay suficiente muestra pareja entre ambos lados.", "There is not enough balanced sample on both sides yet.");
  }
  if ((variant.topMatchups[0]?.share ?? 0) >= 35 && variant.topMatchups[0]?.championName !== baseline.topMatchups[0]?.championName) {
    return copy(locale, "Parte del edge puede venir del matchup que absorbi\xF3 esta variante.", "Part of the edge may come from the matchup allocation absorbed by this variant.");
  }
  if (Math.abs(variant.avgDurationMinutes - baseline.avgDurationMinutes) >= 4) {
    return copy(locale, "La duraci\xF3n media cambi\xF3 bastante entre p\xE1ginas, as\xED que conviene leer el edge con cuidado.", "Average duration changed a lot between pages, so the edge needs a careful read.");
  }
  if (Math.abs(variant.earlyStateProfile.volatile - baseline.earlyStateProfile.volatile) >= 20) {
    return copy(locale, "La p\xE1gina no est\xE1 entrando al mismo tipo de early, as\xED que no toda la diferencia es de runas.", "The page is not entering the same type of early, so not all the difference belongs to the runes.");
  }
  return null;
}
function buildBestWhen(baseline, variant, locale) {
  if (variant.earlyStatePerformance.stable.games >= 2 && variant.earlyStatePerformance.stable.winRate >= baseline.earlyStatePerformance.stable.winRate + 8) {
    return copy(locale, "Cuando el early llega limpio y pod\xE9s jugar una versi\xF3n ordenada del campe\xF3n.", "When the early game stays clean and you can play an orderly version of the champion.");
  }
  if (variant.durationPerformance.long.games >= 2 && variant.durationPerformance.long.winRate >= baseline.durationPerformance.long.winRate + 8) {
    return copy(locale, "Cuando la partida suele alargarse y la p\xE1gina escala mejor con el tiempo.", "When the game tends to go longer and the page scales better over time.");
  }
...
function buildAvoidWhen(baseline, variant, locale) {
  if (variant.earlyStatePerformance.volatile.games >= 2 && variant.earlyStatePerformance.volatile.winRate <= baseline.earlyStatePerformance.volatile.winRate - 8) {
    return copy(locale, "Cuando el early se rompe r\xE1pido y necesit\xE1s una p\xE1gina m\xE1s indulgente.", "When the early game breaks quickly and you need a more forgiving page.");
  }
  if (variant.durationPerformance.short.games >= 2 && variant.durationPerformance.short.winRate <= baseline.durationPerformance.short.winRate - 8) {
    return copy(locale, "Cuando la partida se define demasiado temprano y la p\xE1gina no llega a exprimir su valor.", "When the game gets decided too early and the page does not get to extract its value.");
  }
  return null;
}
function buildRecommendation(variant, deltas, locale) {
  if (deltas.winRate >= 3 && deltas.score >= 2 && deltas.runeValue >= 1e3) {
    return copy(locale, `Probala como p\xE1gina de trabajo cuando quieras optimizar output real, no solo comodidad.`, `Use it as a working page when you want to optimize real output, not only comfort.`);
  }
  if (deltas.deathsPre14 <= -0.6 && deltas.laneVolatility <= -5) {
    return copy(locale, "Vale m\xE1s como p\xE1gina de estabilidad: ordena el early y baja el costo de ejecuci\xF3n.", "It looks more valuable as a stability page: it organizes the early game and lowers execution cost.");
  }
  if (variant.avgRuneHealing > 0 || variant.avgRuneShielding > 0) {
    return copy(locale, "Leela como una p\xE1gina de margen y sustain, no solo como una de da\xF1o bruto.", "Read it as a margin/sustain page, not only as a raw damage page.");
  }
  return copy(locale, "Seguila como variante \xFAtil, pero todav\xEDa no como respuesta cerrada para todo contexto.", "Keep it as a useful variant, but not yet as a closed answer for every context.");
}
function buildComparison(baseline, variant, locale) {
  ...
  const contextScore = (Math.min(baseline.games, variant.games) >= 6 ? 0.25 : 0) + (Math.abs(deltas.durationMinutes) <= 2.5 ? 0.15 : 0) + (Math.abs(variant.earlyStateProfile.volatile - baseline.earlyStateProfile.volatile) <= 12 ? 0.15 : 0);
  ...
  const summary = deltas.winRate >= 0 ? copy(locale, `${variant.compactLabel} est\xE1 ${Math.abs(deltas.winRate).toFixed(1)} pts arriba en WR, ${Math.abs(deltas.score).toFixed(1)} arriba en score y ${Math.abs(deltas.runeValue)} arriba en valor de runa.`, `${variant.compactLabel} is ${Math.abs(deltas.winRate).toFixed(1)} pts up in WR, ${Math.abs(deltas.score).toFixed(1)} up in score and ${Math.abs(deltas.runeValue)} up in rune value.`) : copy(locale, `${variant.compactLabel} est\xE1 ${Math.abs(deltas.winRate).toFixed(1)} pts abajo en WR y ${Math.abs(deltas.score).toFixed(1)} abajo en score.`, `${variant.compactLabel} is ${Math.abs(deltas.winRate).toFixed(1)} pts down in WR and ${Math.abs(deltas.score).toFixed(1)} down in score.`);
  return {
    baseline,
    variant,
    evidenceTier,
...
function buildKeystoneWorkbench(matches, locale) {
  ...
  const variants = Array.from(byVariant.values()).map((groupedMatches) => aggregateVariant(groupedMatches)).sort((left, right) => right.games - left.games || right.winRate - left.winRate);
  const baseline = variants[0];
  const comparisons = variants.slice(1).filter((variant) => variant.games >= 2).map((variant) => buildComparison(baseline, variant, locale)).sort((left, right) => {
    const tierWeight = { strong: 3, weak: 2, hypothesis: 1 };
    return tierWeight[right.evidenceTier] - tierWeight[left.evidenceTier] || right.variant.games - left.variant.games || Math.abs(right.deltas.score) - Math.abs(left.deltas.score);
  });
  return {
...
function championHeadline(championName, comparisons, locale) {
  ...
  if (!strongest) {
    return copy(locale, `${championName}: todav\xEDa no hay una variante cerrada.`, `${championName}: there is still no closed rune variant.`);
  }
  return copy(locale, `${championName}: ${strongest.variant.compactLabel} es la lectura m\xE1s \xFAtil hoy.`, `${championName}: ${strongest.variant.compactLabel} is the most useful read today.`);
}
function championDecisionNote(comparisons, locale) {
  ...
  return weak ? copy(locale, "Todav\xEDa sirve m\xE1s como decisi\xF3n contextual que como default absoluto.", "For now this works better as a contextual decision than as an absolute default.") : copy(locale, "Todav\xEDa hace falta m\xE1s repetici\xF3n para una decisi\xF3n real de optimizaci\xF3n.", "More repetition is still needed for a real optimization decision.");
}
function buildChampionRuneWorkbench(dataset, locale = "es") {
  ...
  return Array.from(byChampion.entries()).map(([championName, championMatches]) => {
    const byKeystone = /* @__PURE__ */ new Map();
    ...
    const keystones = Array.from(byKeystone.values()).filter((matches) => matches.length >= 2).map((matches) => buildKeystoneWorkbench(matches, locale)).sort((left, right) => right.totalGames - left.totalGames);
  ...
  }).filter((champion) => champion.games >= 3 && champion.keystones.length).sort((left, right) => right.games - left.games);
}
export {
  buildChampionRuneWorkbench
};
