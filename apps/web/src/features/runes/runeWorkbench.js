...
function buildSignalNote(baseline, variant, locale) {
...
function buildComparison(baseline, variant, locale) {
    ...
    const signalScore = Math.max(Math.abs(deltas.winRate) / 12, Math.abs(deltas.score) / 4, Math.abs(deltas.damageToChampions) / 3000, Math.abs(deltas.csAt15) / 8, Math.abs(deltas.deathsPre14) / 1.3);
    const contextScore = ((buildContextNote(baseline, variant, locale) ? 0 : 0.2)
