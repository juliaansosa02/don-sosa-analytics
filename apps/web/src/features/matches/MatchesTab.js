...
export function MatchesTab({ dataset, locale = 'es' }) {
    ...
    return (_jsx("div", { style: { display: 'grid', gap: 16 }, children: matches.map((match) => {
            ...
            return (_jsxs("article", { style: {
                ...
                }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'start', flexWrap: 'wrap' }, children: [_jsx(ChampionIdentity, { championName: match.championName, version: dataset.ddragonVersion, subtitle: `${formatDetailDate(match.gameCreation, locale)} · ${getQueueLabel(match.queueId)} · ${formatMatchDuration(match.gameDurationSeconds, locale)}`, meta: _jsxs(_Fragment, { children: [_jsx(Badge, { tone: match.win ? 'low' : 'high', children: match.win ? (locale === 'en' ? 'Win' : 'Victoria') : (locale === 'en' ? 'Loss' : 'Derrota') }), _jsx(Badge, { tone: toneToBadgeTone(quickRead.tone), children: quickRead.toneLabel }), _jsx(Badge, { tone: toneToBadgeTone(quickRead.tone), children: quickRead.impactLabel })] }) }), _jsxs("div", { style: { display: 'grid', gap: 10, justifyItems: 'end' }, children: [_jsxs("div", { style: { display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'end' }, children: [_jsx(Badge, { children: match.opponentChampionName ? `vs ${formatChampionName(match.opponentChampionName)}` : (locale === 'en' ? 'Opponent unknown' : 'Rival sin detectar') }), _jsx(Badge, { children: locale === 'en' ? getRoleLabel(match.role || dataset.summary.primaryRole || 'ALL') : getRoleLabel(match.role || dataset.summary.primaryRole || 'ALL') }), _jsx(Badge, { children: `${match.kills}/${match.deaths}/${match.assists}` }), _jsx(Badge, { children: `${formatDecimal(match.killParticipation)}% KP` })] }), _jsx("button", { type: "button", onClick: () => setExpandedMatchId(expanded ? null : match.matchId), style: detailToggleStyle, children: expanded
