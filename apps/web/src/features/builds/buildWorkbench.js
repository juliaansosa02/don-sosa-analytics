...
function estimateTrackedWindows(match, catalog, locale) {
    ...
    if (cullPurchase) {
        ...
        if (payoffPoint)
            tracked.push({ label: copy(locale, 'Cull paga', 'Cull pays off'), minute: payoffPoint.minute, coverage: 1 });
    }
    ...
    if (sealPurchase) {
        ...
        if (firstTakedown)
            tracked.push({ label: copy(locale, 'Dark Seal arranca', 'Dark Seal activates'), minute: firstTakedown, coverage: 1 });
    }
    ...
    if (hubrisPurchase) {
        ...
        if (firstTakedown)
            tracked.push({ label: copy(locale, 'Hubris activa', 'Hubris activates'), minute: firstTakedown, coverage: 1 });
    }
    return tracked;
}
...
function buildCompFitLabel(family, pressureProfile, catalog, locale) {
    ...
    if (pressureProfile === 'heavy_ap' && tags.has('SpellBlock'))
        return copy(locale, 'build con respuesta visible a presión AP', 'build shows a visible answer into AP pressure');
    if (pressureProfile === 'heavy_ad' && tags.has('Armor'))
        return copy(locale, 'build con respuesta visible a presión AD', 'build shows a visible answer into AD pressure');
    if (pressureProfile === 'double_frontline' && (tags.has('ArmorPenetration') || tags.has('MagicPenetration')))
        return copy(locale, 'path alineado para romper frontline', 'path looks aligned to break frontline');
    if (tags.has('CriticalStrike') || tags.has('AttackSpeed'))
        return copy(locale, 'path de snowball / greed', 'snowball / greed path');
...
function buildMissingResponseLabel(family, pressureProfile, catalog, locale) {
    const tags = new Set([...family.topItems, family.bootsId].filter((itemId) => typeof itemId === 'number').flatMap((itemId) => itemTags(itemId, catalog)));
    if (pressureProfile === 'heavy_ap' && !tags.has('SpellBlock'))
        return copy(locale, 'faltó una respuesta visible de MR / botas menos greedy', 'missing a visible MR answer / less greedy boots');
    if (pressureProfile === 'heavy_ad' && !tags.has('Armor'))
        return copy(locale, 'faltó una respuesta visible de armor / tempo defensivo', 'missing a visible armor / defensive tempo answer');
    if (pressureProfile === 'double_frontline' && !tags.has('ArmorPenetration') && !tags.has('MagicPenetration'))
        return copy(locale, 'faltó más shred o penetración para frontline', 'missing more shred or penetration into frontline');
    return null;
}
...
function buildAvoidWhen(baseline, variant, locale) {
    ...
    if (variant.missingResponseLabel)
        return variant.missingResponseLabel;
    return null;
}
function buildComparison(baseline, variant, catalog, locale) {
    ...
    const signalScore = Math.max(Math.abs(deltas.winRate) / 12, Math.abs(deltas.score) / 4, Math.abs(deltas.damageToChampions) / 3000, Math.abs(deltas.totalDamage) / 6000, Math.abs(deltas.goldDiffAt15) / 350, Math.abs(deltas.laneVolatility) / 10, Math.abs(deltas.firstItemMinute ?? 0) / 3);
...
function championHeadline(championName, comparisons, locale) {
    ...
    if (!strongest)
        return copy(locale, `${championName}: todavía no hay una familia alternativa cerrada.`, `${championName}: there is still no closed alternative family.`);
...
function championDecisionNote(baseline, comparisons, locale) {
    if (!baseline)
        return copy(locale, 'Todavía no hay baseline suficiente.', 'There is still not enough baseline.');
    ...
    if (baseline.missingResponseLabel)
        return copy(locale, `Watchpoint base: ${baseline.missingResponseLabel}.`, `Baseline watchpoint: ${baseline.missingResponseLabel}.`);
