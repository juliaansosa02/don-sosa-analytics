...
export function BuildsTab({ dataset, locale = 'es' }) {
    ...
    return (_jsxs("div", { style: { display: 'grid', gap: 18 }, children: [_jsx(Card, { title: locale === 'en' ? 'Builds / items lab' : 'Lab de builds / items', subtitle: locale === 'en'
                    ...
                    : 'No es una tabla de “most picked”. Es un workspace por campeón para familias de build, timings de items, leverage por item e hipótesis de comp-fit.', children: _jsxs("div", { style: { display: 'grid', gap: 16 }, children: [_jsxs("div", { style: topSummaryGridStyle, children: [_jsx(SummaryBox, { label: locale === 'en' ? 'Champions with build families' : 'Campeones con familias de build', value: String(workspace.champions.length), hint: locale === 'en' ? 'At least one baseline plus an alternative path.' : 'Al menos un baseline más un path alternativo.' }), _jsx(SummaryBox, { label: locale === 'en' ? 'Strong build reads' : 'Lecturas fuertes de build', value: String(strongReads), hint: locale === 'en' ? 'Families with decent sample and a real change in output.' : 'Familias con muestra decente y un cambio real de output.' }), _jsx(SummaryBox, { label: locale === 'en' ? 'Directional reads' : 'Lecturas direccionales', value: String(weakReads + comparisons.length - strongReads - weakReads), hint: locale === 'en' ? 'Useful for watchlists, not yet a closed answer.' : 'Sirven para watchlists, no todavía como respuesta cerrada.' })] }), _jsx("div", { style: notePanelStyle, children: locale === 'en'
                                ...
                                : 'Cada campeón recibe un baseline de build, familias alternativas, deltas de timing, ventanas seguidas para items de stack/activación y una primera heurística de comp-fit. Cuando la muestra es flaca, la UI lo dice explícitamente.' })] }) }), workspace.champions.map((champion) => (_jsx(Card, { title: champion.championName, subtitle: locale === 'en'
                    ? `${champion.games} visible games. The baseline is the most repeated build family for this champion in the current sample.`
                    : `${champion.games} partidas visibles. El baseline es la familia de build más repetida para este campeón en la muestra actual.`, children: _jsxs("div", { style: { display: 'grid', gap: 18 }, children: [_jsx(ChampionIdentity, { championName: champion.championName, version: dataset.ddragonVersion, subtitle: locale === 'en'
                                ? 'The point is to understand what changes when the build changes, not just what gets clicked more often.'
                                : 'La idea es entender qué cambia cuando cambia la build, no solo qué se cliquea más seguido.', meta: _jsx(Badge, { children: locale === 'en' ? `${champion.buildFamilies.length} visible families` : `${champion.buildFamilies.length} familias visibles` }) }), champion.baseline ? _jsx(BaselineBuildPanel, { dataset: dataset, baseline: champion.baseline, locale: locale }) : null, champion.comparisons.length ? (_jsx("div", { style: { display: 'grid', gap: 12 }, children: champion.comparisons.slice(0, 3).map((comparison) => (_jsx(BuildComparisonCard, { dataset: dataset, comparison: comparison, locale: locale }, `${comparison.variant.key}-${comparison.variant.label}`))) })) : (_jsx("div", { style: emptyStateStyle, children: locale === 'en'
...
function BaselineBuildPanel({ dataset, baseline, locale }) {
    return (_jsxs("div", { style: baselineCardStyle, children: [_jsxs("div", { style: { display: 'grid', gap: 6 }, children: [_jsx("div", { style: sectionLabelStyle, children: locale === 'en' ? 'Baseline family' : 'Familia baseline' }), _jsx("div", { style: { color: '#f4fbff', fontSize: 20, fontWeight: 800 }, children: baseline.label }), _jsx("div", { style: { color: '#90a0b5', lineHeight: 1.6 }, children: locale === 'en'
                            ...
                            : `${baseline.games} partidas · ${formatPercent(baseline.winRate)} WR · ${formatDecimal(baseline.avgScore)} de score medio` })] }), _jsxs("div", { style: metricsGridStyle, children: [_jsx(MetricBox, { label: locale === 'en' ? 'Total damage' : 'Daño total', value: formatInteger(baseline.avgTotalDamage) }), _jsx(MetricBox, { label: locale === 'en' ? 'Damage to champs' : 'Daño a champs', value: formatInteger(baseline.avgDamageToChampions) }), _jsx(MetricBox, { label: "CS", value: formatInteger(baseline.avgCs) }), _jsx(MetricBox, { label: "CS@15", value: formatDecimal(baseline.avgCsAt15) }), _jsx(MetricBox, { label: locale === 'en' ? '1st item' : '1er item', value: baseline.avgFirstItemMinute !== null ? `${formatDecimal(baseline.avgFirstItemMinute)}m` : '—' }), _jsx(MetricBox, { label: locale === 'en' ? '2nd item' : '2do item', value: baseline.avgSecondItemMinute !== null ? `${formatDecimal(baseline.avgSecondItemMinute)}m` : '—' }), _jsx(MetricBox, { label: locale === 'en' ? 'Boots' : 'Botas', value: baseline.avgBootsMinute !== null ? `${formatDecimal(baseline.avgBootsMinute)}m` : '—' })] }), _jsxs("div", { style: { display: 'flex', gap: 8, flexWrap: 'wrap' }, children: [baseline.topItems.map((itemId) => (_jsx(ItemPill, { dataset: dataset, itemId: itemId }, `${baseline.key}-${itemId}`))), baseline.bootsId ? _jsx(ItemPill, { dataset: dataset, itemId: baseline.bootsId }) : null, _jsxs("div", { style: contextPillStyle, children: [_jsx("strong", { children: locale === 'en' ? 'Enemy pressure' : 'Presión rival' }), _jsx("span", { children: baseline.pressureProfile.replaceAll('_', ' ') })] })] }), baseline.trackedWindows.length ? (_jsx("div", { style: { display: 'flex', gap: 8, flexWrap: 'wrap' }, children: baseline.trackedWindows.map((window) => (_jsxs("div", { style: trackingPillStyle, children: [_jsx("strong", { children: window.label }), _jsxs("span", { children: [formatDecimal(window.minute), "m \u00B7 ", window.coverage, "%"] })] }, `${baseline.key}-${window.label}`))) })) : null] }));
}
function BuildComparisonCard({ dataset, comparison, locale }) {
    return (_jsxs("div", { style: comparisonCardStyle, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', alignItems: 'start' }, children: [_jsxs("div", { style: { display: 'grid', gap: 5 }, children: [_jsx("div", { style: sectionLabelStyle, children: locale === 'en' ? 'Alternative family' : 'Familia alternativa' }), _jsx("div", { style: { color: '#eef4ff', fontSize: 18, fontWeight: 800 }, children: comparison.variant.label }), _jsxs("div", { style: { display: 'flex', gap: 8, flexWrap: 'wrap' }, children: [comparison.variant.topItems.map((itemId) => (_jsx(ItemPill, { dataset: dataset, itemId: itemId }, `${comparison.variant.key}-${itemId}`))), comparison.variant.bootsId ? _jsx(ItemPill, { dataset: dataset, itemId: comparison.variant.bootsId }) : null] })] }), _jsx(Badge, { tone: evidenceTone(comparison.evidenceTier), children: evidenceBadgeLabel(comparison.evidenceTier, locale) })] }), _jsxs("div", { style: deltaGridStyle, children: [_jsx(DeltaBox, { label: "WR", value: formatSignedNumber(comparison.deltas.winRate, 1, '%') }), _jsx(DeltaBox, { label: locale === 'en' ? 'Score' : 'Score', value: formatSignedNumber(comparison.deltas.score) }), _jsx(DeltaBox, { label: locale === 'en' ? 'Total dmg' : 'Daño total', value: formatSignedNumber(comparison.deltas.totalDamage, 0) }), _jsx(DeltaBox, { label: locale === 'en' ? 'Champ dmg' : 'Daño champs', value: formatSignedNumber(comparison.deltas.damageToChampions, 0) }), _jsx(DeltaBox, { label: "CS", value: formatSignedNumber(comparison.deltas.cs, 0) }), _jsx(DeltaBox, { label: locale === 'en' ? '1st item' : '1er item', value: comparison.deltas.firstItemMinute !== null ? formatSignedNumber(comparison.deltas.firstItemMinute, 1, 'm') : '—' }), _jsx(DeltaBox, { label: locale === 'en' ? '2nd item' : '2do item', value: comparison.deltas.secondItemMinute !== null ? formatSignedNumber(comparison.deltas.secondItemMinute, 1, 'm') : '—' }), _jsx(DeltaBox, { label: locale === 'en' ? 'Boots' : 'Botas', value: comparison.deltas.bootsMinute !== null ? formatSignedNumber(comparison.deltas.bootsMinute, 1, 'm') : '—' })] }), _jsxs("div", { style: { display: 'grid', gap: 8 }, children: [_jsx("div", { style: { color: '#f2f8ff', fontWeight: 700, lineHeight: 1.65 }, children: comparison.summary }), comparison.pressureNote ? _jsx("div", { style: { color: '#9ba9bc', lineHeight: 1.6 }, children: comparison.pressureNote }) : null, comparison.recommendation ? _jsx("div", { style: warningCalloutStyle, children: comparison.recommendation }) : null, _jsx("div", { style: { color: '#7f8da2', fontSize: 12, lineHeight: 1.6 }, children: evidenceExplanation(comparison.evidenceTier, locale, comparison.baseline.games + comparison.variant.games, comparison.constraint) })] })] }));
}
function ItemSignalColumn({ title, emptyLabel, items, dataset, locale }) {
    return (_jsxs("div", { style: signalColumnStyle, children: [_jsx("div", { style: sectionLabelStyle, children: title }), items.length ? (_jsx("div", { style: { display: 'grid', gap: 10 }, children: items.map((item) => (_jsxs("div", { style: itemImpactCardStyle, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap', alignItems: 'center' }, children: [_jsxs("div", { style: { display: 'flex', gap: 10, alignItems: 'center' }, children: [_jsx(ItemPill, { dataset: dataset, itemId: item.itemId, compact: true }), _jsxs("div", { style: { display: 'grid', gap: 3 }, children: [_jsx("div", { style: { color: '#eef4ff', fontWeight: 700 }, children: item.name }), _jsxs("div", { style: { color: '#8d9bb0', fontSize: 12 }, children: [item.games, " \u00B7 ", formatDecimal(item.usageShare), "% \u00B7 ", item.utilityLabel] })] })] }), _jsx(Badge, { tone: evidenceTone(item.evidenceTier), children: evidenceBadgeLabel(item.evidenceTier, locale) })] }), _jsxs("div", { style: itemImpactMetricsStyle, children: [_jsx(DeltaBox, { label: "WR", value: formatSignedNumber(item.winRateDelta, 1, '%') }), _jsx(DeltaBox, { label: locale === 'en' ? 'Score' : 'Score', value: formatSignedNumber(item.scoreDelta) }), _jsx(DeltaBox, { label: locale === 'en' ? 'Champ dmg' : 'Daño champs', value: formatSignedNumber(item.damageDelta, 0) }), _jsx(DeltaBox, { label: locale === 'en' ? 'Completion' : 'Completa', value: item.avgCompletionMinute !== null ? `${formatDecimal(item.avgCompletionMinute)}m` : '—' })] })] }, `${title}-${item.itemId}`))) })) : (_jsx("div", { style: emptyStateStyle, children: emptyLabel }))] }));
}
