...
export function Card({ title, subtitle, children }) {
    return (_jsxs("section", { style: { display: 'grid', alignContent: 'start', gap: 16, height: '100%', background: 'rgba(8,12,20,0.94)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: 20, boxShadow: '0 16px 42px rgba(0,0,0,0.16)' }, children: [_jsxs("div", { style: { display: 'grid', gap: 8 }, children: [_jsx("h3", { style: { margin: 0, fontSize: 18, letterSpacing: '-0.02em' }, children: title }), subtitle ? _jsx("p", { style: { margin: 0, color: '#7f8898', lineHeight: 1.6 }, children: subtitle }) : null] }), children] }));
}
...
export function Badge({ children, tone = 'default' }) {
    ...
    return _jsx("span", { style: { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '5px 8px', borderRadius: 999, background: tones[tone].background, color: tones[tone].color, fontSize: 11, fontWeight: 700, letterSpacing: 0, lineHeight: 1.2, textAlign: 'center', border: '1px solid rgba(255,255,255,0.045)' }, children: children });
}
